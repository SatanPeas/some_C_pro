...
bool CIOCPModel::_InitializeIOCP()
{
	...
	for (int i = 0; i < m_nThreads; i++)
	{
		...
		m_phWorkerThreads[i] = ::CreateThread(0, 0, _WorkerThread, (void *)pThreadParams, 0,
...
bool CIOCPModel::_PostAccept( PER_IO_CONTEXT* pAcceptIoContext )
{
	...
	if(FALSE == m_lpfnAcceptEx( m_pListenContext->m_Socket, pAcceptIoContext->m_sockAccept, p_wbuf->buf,
			p_wbuf->len - ((sizeof(SOCKADDR_IN)+16)*2), sizeof(SOCKADDR_IN)+16, sizeof(SOCKADDR_IN)+16, &dwBytes, p_ol))
	{
...
bool CIOCPModel::_PostRecv( PER_IO_CONTEXT* pIoContext )
{
	...
	// 初始化完成后，，投递WSARecv请求
	int nBytesRecv = WSARecv( pIoContext->m_sockAccept, p_wbuf, 1, &dwBytes, &dwFlags, p_ol,
...
{
	EnterCriticalSection(&m_csContextList);
	vector<PER_SOCKET_CONTEXT*>::iterator tempit;

	for( tempit = m_arrayClientContext.begin(); tempit < m_arrayClientContext.end(); tempit++)
	{
		if( pSocketContext == *tempit )
		{
			RELEASE( pSocketContext );
			m_arrayClientContext.erase(tempit);
			break;
		}
	}
