...
{  
	...
	~_PER_SOCKET_CONTEXT()
	{
		...
		// 释放掉所有的IO上下文数据
		vector<_PER_IO_CONTEXT*>::iterator tempit;
		for( tempit = m_arrayIoContext.begine(); tempit < m_arrayIoContext.end(); tempit++ )
		{
			delete m_arrayIoContext.erase(tempit);
		}
